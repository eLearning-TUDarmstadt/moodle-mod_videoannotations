...
function random_lipsum($amount = 1, $what = 'paras', $start = 0) {
    return simplexml_load_file('http://www.lipsum.com/feed/xml?amount='.$amount.'&what='.$what.'&start='.$start.'&lang=de')->lipsum;
}

$numberOfAnnotations = 5; //rand(1, 1);
for ($i = 1; $i <= $numberOfAnnotations; $i++) {
    $user_data_json = file_get_contents('https://randomuser.me/api/');
    $user = json_decode($user_data_json);
    $user = $user->results[0];
    //echo "<pre>" . print_r($user, true) . "</pre>";
    $paragraphs = rand(1, 4);
    $data['annotations'][] = [
        'user' => ucfirst($user->name->first) . " " . ucfirst($user->name->last),
        'timeago' => 'vor ' . $paragraphs . ' Stunden',
        'user_pic' => $user->picture->large,
        'subject' => random_lipsum($paragraphs, 'words'),
        //'content' => file_get_contents('http://loripsum.net/api/' . $paragraphs)
        'content' => random_lipsum($paragraphs)
    ];
}
